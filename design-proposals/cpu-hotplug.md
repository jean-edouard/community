...
Under the `cpu` entry, it will be possible to define a maximum number of sockets. That number is needed by LibVirt and will default to 4 times the initial number of sockets if not set.
...
The VMI status must indicate the current and desired number of CPU sockets. These values need to be propagated back to the VM status to allows the user to have an indication of when their changes applied to the running VMI.

The user should be able to inspect the VM.Status values below to determine the state of their changes. This is similar in concept to someone inspecting a Deployments status to determine the desired number of pods vs the number of pods that are available.

```
// The declared number of sockets the vm is converging on
vmi.Status.Resources.Sockets.Desired
vm.Status.Resources.Sockets.Desired
...
## Phase 3 (future): Using the [KubeVirt vertical scaling API](https://github.com/kubernetes/enhancements/tree/master/keps/sig-node/1287-in-place-update-pod-resources ) once it reaches a more mature state
